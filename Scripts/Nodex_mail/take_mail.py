            ...
            if not part.is_multipart():
                ...
                if name:
                    ...
                    if content_type == 'text/html':
                        ...
                        # # soup = BeautifulSoup(str(txt, 'gbk'), 'html.parser')
                        ...
                        with open('main_body.txt', 'w') as fw:
                            fw.write(str(txt, 'utf-8'))
                        print(str(txt, 'utf-8'))
    def front(self, conn):
        """
        使用163邮箱，[Skrill下载](https://www.gendan5.com/wallet/Skrill.html)必须在select之前上传客户端身份信息,否则报错：command SEARCH illegal in state AUTH, only allowed in states SELECTED
        :param conn: imap连接
        :return:
        """
...
if __name__ == '__main__':
    IMAP = IMAP('helloword7658@aveinmail521.com', '1111qqqq', 'out-sea.com')
    conn = IMAP.login()
    IMAP.front(conn)
